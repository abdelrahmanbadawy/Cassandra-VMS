...
import com.datastax.shaded.netty.util.Version;
...
import org.apache.cassandra.cql3.statements.Restriction.EQ;
...
public class CommitLog implements CommitLogMBean
{
	...
	public ReplayPosition add(Mutation mutation)
	{
		...
		if(!mutation.getKeyspaceName().equals("system")){
			extractDeserializedInfoFromCL(mutation);
		}
	...
	private void extractDeserializedInfoFromCL(Mutation mutation) {

		String keyspace = mutation.getKeyspaceName();
		String table = "";
		ArrayList<String> columns = new ArrayList<String>();
		ArrayList<String> values = new ArrayList<String>();
		long tid = mutation.hashCode();
		...
		for (Map.Entry<UUID, ColumnFamily> entry : mutation.getModifications().entrySet()){
			...
			table = Schema.instance.getCFMetaData(entry.getKey()).cfName;
			...
			case "org.apache.cassandra.db.marshal.UTF8Type":
				byte[] bytes = null;
				if(mutation.key().hasArray()) {
					bytes = mutation.key().array();
				} else {
					bytes = new byte[mutation.key().remaining()];
					mutation.key().get(bytes);
				}
				try {
					pkValue = "'"+new String(bytes, "UTF-8")+"'";
			...
			for (Cell cell : cf){
				try {
					...
					if(!cell.name().equals(pkName) && i!=0){
						...
						if(cellType.equals("org.apache.cassandra.db.marshal.UTF8Type"))
							values.add("'"+cfm.getValueValidator(cell.name()).getString(cell.value())+"'");
						else if(cfm.comparator.getString(cell.name()).equals("stream")){
							values.add("0x"+cfm.getValueValidator(cell.name()).getString(cell.value()).trim());
						}else{
							values.add(cfm.getValueValidator(cell.name()).getString(cell.value()));
						}
					}
	...
	private JSONObject convertInsertUpdateToJSON(String ks,
		...
		for (int i = 0; i < columns.size(); i++) {
			set_data.put(columns.get(i).replaceAll(" ", ""), values.get(i));
		}
	...
	private JSONObject convertDeleteToJSON(String ks,
			String table, String condition_columns,
			String inValues, long tid) {

		JSONObject jsonObject = new JSONObject();

		jsonObject.put("type", "delete-row");
		jsonObject.put("tid", tid);
		jsonObject.put("keyspace", ks);
		jsonObject.put("table", table);
		...
		condition.put(condition_columns, inValues);
